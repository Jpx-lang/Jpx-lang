...
class ImportHandler:
    ...
    def find_module(self, module_name):
        ...
        Urutan pencarian:
          1. script_dir/module.jpx
          2. script_dir/module.py
          3. lib/module.jpx (cwd)
          4. lib/module.py
          5. JPX_PATH entries (jpx lalu py)
          6. library/module.jpx (built-in)
          7. library/module.py
        ...
        for path in self.get_search_paths():
            # Determine priority: di user paths (.jpx first), di builtin (.py first)
        ...
        return None  # caller can use self._last_searched if needed
    ...
    def _import_one(self, name):
        ...
        # Handle module.attr (single attribute from module)
        if '.' in name:
            parts = name.split('.')
            if len(parts) != 2:
                raise exceptions.JPXImportError(
                    f"Invalid import '{name}': only 'module.attr' "
                    f"syntax is supported (no nested attributes)"
                )
            ...
            # Cek exports dict dulu (untuk .py module dengan pattern `exports = {...}`)
            value = None
            found = False
        ...
        for path in self.get_search_paths():
            ...
            for filename in os.listdir(path):
                ...
                if ext in ('.py', '.jpx') and name not in seen:
                    seen[name] = (ext, os.path.join(path, filename))
        return seen
