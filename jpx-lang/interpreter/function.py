...
class JPXFunction:
    ...
    def __init__(self, name, params, body, interpreter, is_anonymous=False, closure_env=None):
        ...
        if closure_env is None:
            ...
            # Recursive support: function harusnya bisa call dirinya sendiri
            # dan sibling functions di scope yang sama. Karena closure_env
            # di-snapshot SEBELUM function di-add ke env, kita perlu add
            # self-reference manual agar recursive call jalan.
            if name:
                self.closure_env[name] = self
        else:
