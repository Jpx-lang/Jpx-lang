...
class EvalHandler:
    ...
    def process_string(self, s):
        """Handle escape sequences dan $variable interpolation di string literal.

        Sebelumnya hanya handle $variable. Escape sequences seperti \\n, \\t,
        \\", \\\\ tetap mentah di output. Sekarang kita unescape dulu, lalu
        ...
        while i < len(s):
            ...
            if ch == '\\' and i + 1 < len(s):
                ...
                if nxt == 'n':
                    ...
                    # Escape $ agar tidak di-interpolate
                    result.append('$')
                else:
        ...
        # 2. Interpolate ${expr} (expression) dan $variable
        # ${expr} = evaluasi expression apa pun (method call, arithmetic, dll)
        # $var = interpolasi variable sederhana (backward compat)

        # Pertama: handle ${expr} — HANYA jika string mengandung ${
        # (optimasi: skip character-by-character scan untuk string biasa)
        if '${' in unescaped:
            result = []
            i = 0
            while i < len(unescaped):
                if i + 1 < len(unescaped) and unescaped[i] == '$' and unescaped[i+1] == '{':
                    # Cari matching }
                    j = i + 2
                    depth = 1
                    while j < len(unescaped) and depth > 0:
                        if unescaped[j] == '{':
                            depth += 1
                        elif unescaped[j] == '}':
                            depth -= 1
                        ...
                        j += 1
                    if depth == 0:
                        expr_str = unescaped[i+2:j].strip()
                        try:
                            val = self.eval_expression(expr_str)
                            result.append(str(val) if val is not None else "")
                        except Exception as e:
                            result.append(f"<error:{e}>")
                        i = j + 1
                        continue
                result.append(unescaped[i])
                i += 1
            interpolated = ''.join(result)
        else:
            interpolated = unescaped

        # Kedua: handle $variable (simple, backward compat)
        def repl_var(match):
        ...
        return re.sub(pattern, repl_var, interpolated)
    ...
    def eval_expression(self, expr):
        ...
        # Pipe operator: `input |> fn1 |> fn2`
        # Evaluasi left-to-right: fn2(fn1(input))
        # Track string dan depth agar `|>` di dalam string/paren diabaikan.
        if '|>' in expr:
            in_string = False
            quote_char = None
            depth = 0
            ...
            while i < len(expr) - 1:
                ...
                if ch in '"\'':
                    ...
                    depth -= 1
                elif depth == 0 and ch == '|' and expr[i+1] == '>':
                    pipe_positions.append(i)
                i += 1

            if pipe_positions:
                # Split by |> dan evaluasi bertahap
                parts = []
                prev = 0
                ...
                # Evaluate first part
                result = self.eval_expression(parts[0])
                # Pipe through subsequent parts
                for i in range(1, len(parts)):
                    fn_expr = parts[i]
                    fn = self.eval_expression(fn_expr)
        ...
        # Syntax:
        #   IDENT.method(args)        — string variable
        #   "literal".method(args)    — string literal
        #   arr[i].method(args)       — index access yang return string
        #   obj.field.method(args)    — property access yang return string
        #
        # Kita ekstrak "obj part" (sebelum .method) dan eval terlebih dahulu.
        # Kalau hasilnya string, pakai string method. Kalau bukan, fall through.
        string_method_match = re.match(
            r'^(.*?)\s*\.\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*\((.*)\)\s*$',
            expr, re.DOTALL
        )
        if string_method_match:
            obj_str = string_method_match.group(1).strip()
            method_name = string_method_match.group(2)
            args_str = string_method_match.group(3)

            # Hanya coba kalau obj_str adalah expression yang valid
            # (bukan nested call kompleks — itu ditangani function call path)
            # Skip kalau ada operator di obj_str (mis. "a + b".method())
            # Kecuali index/property access yang valid
            should_try = False
            if obj_str.startswith('"') and obj_str.endswith('"'):
                should_try = True
            elif obj_str.startswith("'") and obj_str.endswith("'"):
                should_try = True
            elif re.match(r'^[a-zA-Z_][a-zA-Z0-9_.\[\]]*$', obj_str):
                # IDENT atau IDENT.field atau IDENT[idx] atau IDENT[idx][idx]...
                should_try = True
            ...
            if should_try:
                ...
                if obj_str.startswith('"') and obj_str.endswith('"'):
                    ...
                    try:
                        obj = self.eval_expression(obj_str)
        ...
        if func_match:
            ...
            # Track string state agar `(` atau `)` di dalam string literal
            # tidak dihitung sebagai paren pembuka/penutup.
            depth = 0
            matching_close = -1
            in_str = False
            quote_ch = None
            ...
            if matching_close == len(expr) - 1 and matching_close > open_pos:
                ...
                try:
                    if '.' in func_path:
                        ...
                        # Cek apakah ini class (untuk instantiation via module.ClassName())
                        if isinstance(fn, dict) and fn.get('__jpx_class__'):
                            return self._instantiate_class(fn, args)
                        if callable(fn):
                            if isinstance(fn, JPXFunction):
                                return fn.call(args)
                            return fn(*args)
            ...
            while pos < len(expr):
                ...
                if expr[pos] == '.':
                    ...
                    if ':' in inner and not inner.startswith('"') and not inner.startswith("'"):
                        ...
                        # Convert float ke int untuk slice index (Python strict)
                        if start is not None and isinstance(start, float):
                            start = int(start)
                        if end_v is not None and isinstance(end_v, float):
                            end_v = int(end_v)
                        try:
                            obj = obj[start:end_v]
                        except TypeError:
        ...
        # Supports: x in list, key in dict, substr in str
        # Cek SEBELUM comparison agar `in` tidak tertangkap comparison path.
        # Track string dan depth agar `in` di dalam string/paren diabaikan.
        if ' in ' in expr:
            in_string = False
            depth = 0
            i = 0
            while i < len(expr) - 3:
                ...
                if ch == '"':
                    ...
                    if ch in '([{':
                        ...
                        depth -= 1
                    elif depth == 0 and expr[i:i+4] == ' in ':
                        left = expr[:i].strip()
        ...
        # Supports: x is null, x is true, x is y
        if ' is ' in expr:
            in_string = False
            depth = 0
            i = 0
            while i < len(expr) - 3:
                ...
                if ch == '"':
                    ...
                    if ch in '([{':
                        ...
                        depth -= 1
                    elif depth == 0 and expr[i:i+4] == ' is ':
                        left = expr[:i].strip()
                        right = expr[i+4:].strip()
                        left_val = self.eval_expression(left)
                        right_val = self.eval_expression(right)
                        # `is` = identity check (untuk null/None check)
                        if right.strip() == 'null':
